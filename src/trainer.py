...
class TransformerDDPTrainer:
    ...
    def ddptrain(self,local_rank, nprocs, args):
        ...
        def run_epoch(split,local_rank,Loss):
            is_train = split == 'train'
        ...
        for epoch in range(config.max_epochs):
            self.save_checkpoint()
            Loss=[]
            run_epoch('train',local_rank,Loss)
            Losses=torch.tensor(Loss)
            torch.save(Losses,'epoch_{}.pt'.format(epoch))
            if self.test_dataset is not None:
                test_loss = run_epoch('test',local_rank,Loss)
                print(test_loss)
