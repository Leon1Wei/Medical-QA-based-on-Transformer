...
if __name__=='__main__':
    if Train:
        ...
        dataset=CharDataset(default_config)
        train_size=int(0.7*len(dataset))
        test_size=len(dataset)-train_size
        train_dataset,test_dataset=torch.utils.data.random_split(dataset,[train_size,test_size])
        tconf = TrainerConfig(max_epochs=5 , batch_size=64, learning_rate=2e-4, lr_decay=True, num_workers=20)
        trainer = TransformerDDPTrainer(model, train_dataset, test_dataset, tconf)
